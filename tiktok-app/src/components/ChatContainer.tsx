...
import removeCharacter from '@/util/removeCharacter';
...
const ChatContainer: React.FC<IChatContainer> = ({
    ...
    useEffect(() => {
        ...
        if (currentUserChat._id) {
            (async () => {
                const data = await axios.get(`messages/get?from=${currentUser.id}&to=${currentUserChat._id}`)
                setConservation(data)
            })()
            socket.emit('join-room', Math.abs(removeCharacter(currentUser.id) - removeCharacter(currentUserChat._id)));
        }
    ...
    const sendMessage = async (inputMessage: string) => {
        socket.emit(
            'send-message',
            Math.abs(removeCharacter(currentUser.id) - removeCharacter(currentUserChat._id))
            ,
            {
        ...
        <main>
            <div className="flex-1 flex flex-col border-b-2 h-[440px] overflow-y-auto p-3" >
                {conservation.map((item, index) => (
                    <div key={item._id} ref={scrollRef} className={`${item.sender === currentUser.id ? `flex justify-end ` : ''}`}>
                        <p className={`${item.sender === currentUser.id ? ` text-white bg-cyan-500 p-3 rounded-lg w-fit mt-5` : `text-black mt-5 p-3 rounded-lg w-fit bg-slate-300`}`} key={index}>{item.message}</p>
                    </div>
                ))}
            </div>
            <Formik
