...
function Chat() {
    ...
    const [currentUserChat, setCurrentUserChat] = useState<{ [key: string]: any }>({});
            ...
            <main className="w-[90%] h-[90%] my-auto flex box shadow-lg p-0.5 rounded-lg" style={{ background: 'rgb(248, 248, 248)' }}>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {listUser && listUser.map((user: any) => {
                            if (user?._id !== currentUser?.id) {
                                return (
                                    <section
                                        className={`${currentUserChat._id == user._id ? `bg-slate-200` : `hover:bg-slate-100 `}`}
                                        key={user?._id}
                                        onClick={() => {
                ...
                {currentUserChat && currentUserChat._id && (
                    <article className="w-3/4 bg-white rounded-lg ms-5 flex flex-col">
                        ...
                        <ChatContainer currentUser={currentUser} currentUserChat={currentUserChat} socket={socket} />
                    </article>
                )}
            </main>
        </>
